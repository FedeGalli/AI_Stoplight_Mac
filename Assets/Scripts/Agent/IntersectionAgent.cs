...
public class IntersectionAgent : Agent
{
    ...
    public override void CollectObservations(VectorSensor sensor){
        ...
        sensor.AddObservation(carCount[2]);
        sensor.AddObservation(carCount[3]);
    ...
    {
        if ((colorManager.getColor(Coordinates.North) == colorManager.green && colorManager.getColor(Coordinates.East) == colorManager.red) || (colorManager.getColor(Coordinates.East) == colorManager.green && colorManager.getColor(Coordinates.North) == colorManager.red)){
            ...
            else if (actions.DiscreteActions[0] == 1 && prevCoordinate != Coordinates.East){
                colorManager.activateOnDirection(Coordinates.East);
                prevCoordinate = Coordinates.East;
                greenDirection = Coordinates.East;
                AddReward(-0.5f);
            }
        }   
    ...
    void FixedUpdate()
    {
        ...
        if (colorManager.getColor(Coordinates.North) == colorManager.yellow || colorManager.getColor(Coordinates.East) == colorManager.yellow) {
            listenForReset = true;
        }
        ...
        if (colorManager.getColor(Coordinates.North) != colorManager.red) {
            if (carCount[2] + carCount[3] != 0){ //set a threshold to decide when is appropriate to give a constant negative reward
                AddReward((-0.00001f * (carCount[2] + carCount[3])) * waitTime);
                waitTime += Time.deltaTime;
            }
        }
        if (colorManager.getColor(Coordinates.East) != colorManager.red) {
            if (carCount[0] + carCount[1] != 0){ //set a threshold to decide when is appropriate to give a constant negative reward
                AddReward((-0.00001f * (carCount[0] + carCount[1])) * waitTime);
                waitTime += Time.deltaTime;
            }
        }
    }
    ...
    void flowIntesitivityRandomUpdate(){
        ...
        carFlowManager.flowIntesitivity[2] = 0.15f + Random.Range(-0.20f, 0.15f);
        carFlowManager.flowIntesitivity[3] = 0.15f + Random.Range(-0.20f, 0.15f);

        carFlowManager.turningProbability[0] = 0.3f + Random.Range(-0.2f, 0.1f);
        carFlowManager.turningProbability[1] = 0.3f + Random.Range(-0.2f, 0.1f);
        carFlowManager.turningProbability[2] = 0.3f + Random.Range(-0.2f, 0.1f);
        carFlowManager.turningProbability[3] = 0.3f + Random.Range(-0.2f, 0.1f);

        carFlowManager.rightTurningProbability[0] = 0.8f + Random.Range(-0.1f, 0.1f);
        carFlowManager.rightTurningProbability[1] = 0.8f + Random.Range(-0.1f, 0.1f);
        carFlowManager.rightTurningProbability[2] = 0.8f + Random.Range(-0.1f, 0.1f);
        carFlowManager.rightTurningProbability[3] = 0.8f + Random.Range(-0.1f, 0.1f);
    }
